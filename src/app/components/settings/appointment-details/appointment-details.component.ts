  ...
  template: `
    <div class="page-container">
      <div class="breadcrumb">
        ...
        <span class="breadcrumb-current">איסוף פרטים לתור חדש</span>
      </div>
      
      <div class="page-header">
        <h1>איסוף פרטים לתור חדש</h1>
        <p class="page-description">הגדר שאלות שהבוט ישאל בעת קביעת תור חדש</p>
        <button 
      ...
      <div class="questions-list">
        <div 
          *ngFor="let question of appointmentQuestions; let i = index" 
          class="question-card"
        >
      ...
      <div *ngIf="appointmentQuestions.length === 0" class="empty-state">
        <div class="empty-icon">
          <i class="material-icons">event_note</i>
        </div>
        <h3>אין שאלות מוגדרות</h3>
        <p>עדיין לא הגדרת שאלות לאיסוף פרטים בקביעת תור</p>
        <button 
        ...
        <form class="question-form">
          <div class="form-group">
            ...
            <textarea 
              ...
              name="question"
              required
              placeholder="הכנס את השאלה שהבוט ישאל בעת קביעת תור..."
              rows="3"
              maxlength="500"
            ></textarea>
...
export class AppointmentDetailsComponent implements OnInit {
  ...
  questionToDelete: AppointmentQuestion | null = null;
  ...
  async loadAppointmentQuestions() {
    const { data, error } = await this.supabase.getAppointmentQuestions(this.currentUser.id);
    if (!error && data) {
      this.appointmentQuestions = data;
    }
  }
  ...
  editQuestion(question: AppointmentQuestion) {
  ...
  deleteQuestion(question: AppointmentQuestion) {
  ...
  async confirmDeleteQuestion() {
    ...
    const { error } = await this.supabase.deleteAppointmentQuestion(this.questionToDelete.id);
    
    if (!error) {
      await this.loadAppointmentQuestions();
  ...
  async saveQuestion() {
    ...
    if (this.isEditMode && this.editingQuestionId) {
      const { error } = await this.supabase.updateAppointmentQuestion(this.editingQuestionId, questionData);
      ...
      const { error } = await this.supabase.createAppointmentQuestion(questionData);
